...
class EmailHandler:
    ...
    @classmethod
    def send_verification(cls, email):
        otp = KeyGenerator.get_random_code(number=5, has_digit=True)
        title, content = TextHandler.get_verification_email(otp=otp)
    ...
    def verify_email(cls, email, otp):
        ...
        if not attempts:
            raise ValidationError(error_code=ErrorCode.EMAIL_NOT_VERIFIED, message='email is incorrect')
        valid_otp = DataCache.get_verify_email_otp(email=email)
        if not valid_otp:
            message = 'OTP is expired, please start over again'
            raise ValidationError(error_code=ErrorCode.OTP_IS_EXPIRED, message=message)
        if otp != valid_otp:
            message = f'OTP: {otp} is incorrect'
            raise ValidationError(error_code=ErrorCode.INVALID_OTP, message=message)
        DataCache.del_verify_email_otp(email=email)
